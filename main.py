...
class Game:
    ...
    def get_move_name(self, chosen_move):
        ...
        for move in self.same_move[(chosen_move.new_pos.row, chosen_move.new_pos.col)]:
            if type(move) != Promotion:
                if type(self.board[chosen_move.prev.row][chosen_move.prev.col]) == type(self.board[move.prev.row][move.prev.col]):
        ...
        # same row check
        if col_amb == False:
            return str(self.board[chosen_move.prev.row][chosen_move.prev.col]) + chr(96 + (8 - chosen_move.prev.col))+str(chosen_move)
        ...
        if len(tmp_moves3) <= 1:
            return str(self.board[chosen_move.prev.row][chosen_move.prev.col]) + str(chosen_move.prev.row + 1) + str(chosen_move)
    ...
    def infer_move(self, move, color):
            ...
            if (old_col is None and move_type==Capture) or (old_col is not None and move_type is not Capture):
                ...
                for promotion in promotions:
                    if piece_type == Pawn and promotion.move.new_pos.col == new_col and promotion.move.new_pos.row == new_row:
                        promoted_move = Promotion(move_type(promotion.move.prev,promotion.move.new_pos), promote_to=promoted_piece)
                        self.same_move[(new_row, new_col)].append(promoted_move)
                        return promoted_move
            elif old_col is not None and move_type is Capture:
                promotions = [move for move in candidate_moves if type(move)==Promotion]
                for promotion in promotions:
                    if piece_type == Pawn and promotion.move.new_pos.col == new_col and promotion.move.new_pos.row == new_row and type(promotion.move)==Capture and promotion.move.prev.col == old_col:
                        promoted_move = Promotion(move_type(promotion.move.prev, promotion.move.new_pos), promote_to=promoted_piece)
                        self.same_move[(new_row, new_col)].append(promoted_move)
                        return promoted_move
        ...
        for move in candidate_moves:
            ...
            if move.new_pos.row == new_row and move.new_pos.col == new_col and type(self.board[move.prev.row][move.prev.col]) == piece_type:
                moves_with_same_pos.append(tmp_move_type(move.prev, move.new_pos))
            tmp_move_type = move_type

        final_list = []
        if len(moves_with_same_pos) == 1:
            self.same_move[(moves_with_same_pos[0].new_pos.row, moves_with_same_pos[0].new_pos.col)].append(moves_with_same_pos[0])
...
if __name__ == '__main__':
    ...
    starting_pos = ['a4', 'b5', 'axb5', 'a6','b6','c6','b7','Qc7','bxa8=Q','g5','c3','Qa7','b3', 'd6','Ra5','Qc7','b4','Nh6','Qc2','Nf5','Rxf5','Bxf5','Qe4','Bd7','Qe5','Qb6','Qd4','e5','Qd3','Qxb4','Kd1', 'Qc5','Qc2','a5','Qa6','Qa3','Ke1','Bg7', 'Qb5' ,'Bh6', 'Qd5','Kd8','Qc4','Qa2','Qf5','Be6','Qxg5+','Kd7','Qa6']
    ...
    while 1:
        ...
        if game.turn == Colors.white:
            ...
            # print(game.game_moves_total)
            print(' ' + str(game.move_count) + '.'+game.move_names[-1],end=' ')
        else:
            game.turn = Colors.white
            game.game_moves_total += ' ' + game.move_names[-1]
            # print(game.game_moves_total)
            game.move_count += 1
            print(game.move_names[-1], end=' ')
